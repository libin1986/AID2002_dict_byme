...
def handle(c):
    while True:
        ...
        if data[0] == "E" or data is None:  # 处理客户端的子进程退出
        ...
        if data[0] == "R":  # 处理注册请求
            ...
            do_query(c, data)
        elif data[0] == "H":  # 处理查询里氏记录的请求
            do_history(c, data)

def do_history(c,data):
    """
    1.解析数据，获取查询的用户名
    2.查询数据库history表，根据用户名去查
    3.将数据返回 给客户端,注意粘包sleep（1）
    :param c:
    :param data:
    :return:
    """
    username=data.split(" ")[1]
    result=db.select_history(username)#在数据库中查询历史记录
    for temp in result:
        print(temp,"------------------")
